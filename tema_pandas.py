import pandas as pd
from csv import writer
from matplotlib import *
import matplotlib as plt

#exercitiul 1

dictionar={}
description = ('Country', [
'2011 ', '2012 ', '2013 ', '2014 ', '2015 ', '2016 ', '2017', '2018', '2019 '
])

dataset = [
   ('AL', [': ', ': ', ': ', ': ', ': ', ': ', ': ', '84 ', ': ']),
   ('AT', ['75 ', '79 ', '81 ', '81 ', '82 ', '85 ', '89 ', '89 ', '90 ']),
   ('BA', [': ', ': ', ': ', ': ', ': ', ': ', ': ', '69 ', '72 ']),
   ('BE', ['77 ', '78 ', '80 ', '83 ', '82 ', '85 ', '86 ', '87 ', '90 ']),
   ('BG', ['45 ', '51 ', '54 ', '57 ', '59 ', '64 ', '67 ', '72 ', '75 ']),
   ('CH', [': ', ': ', ': ', '91 ', ': ', ': ', '93 ', ': ', '96 ']),
   ('CY', ['57 ', '62 ', '65 ', '69 ', '71 ', '74 ', '79 ', '86 ', '90 ']),
   ('CZ', ['67 ', '73 ', '73 ', '78 ', '79 ', '82 ', '83 ', '86 ', '87 ']),
   ('DE', ['83 ', '85 ', '88 ', '89 ', '90 ', '92 ', '93 ', '94 ', '95 ']),
   ('DK', ['90 ', '92 ', '93 ', '93 ', '92 ', '94 ', '97 ', '93 ', '95 ']),
   ('EA', ['74 ', '76 ', '79 ', '81 ', '83 ', '85 ', '87 ', '89 ', '90 ']),
   ('EE', ['69 ', '74 ', '79 ', '83 ', '88 ', '86 ', '88 ', '90 ', '90 ']),
   ('EL', ['50 ', '54 ', '56 ', '66 ', '68 ', '69 ', '71 ', '76 ', '79 ']),
   ('ES', ['63 ', '67 ', '70 ', '74 ', '79 ', '82 ', '83 ', '86 ', '91 ']),
('FI', ['84 ', '87 ', '89 ', '90 ', '90 ', '92 ', '94 ', '94 ', '94 ']),
   ('FR', ['76 ', '80 ', '82 ', '83 ', '83 ', '86 ', '86 ', '89 ', '90 ']),
   ('HR', ['61 ', '66 ', '65 ', '68 ', '77 ', '77 ', '76 ', '82 ', '81 ']),
   ('HU', ['63 ', '67 ', '70 ', '73 ', '76 ', '79 ', '82 ', '83 ', '86 ']),
   ('IE', ['78 ', '81 ', '82 ', '82 ', '85 ', '87 ', '88 ', '89 ', '91 ']),
   ('IS', ['93 ', '95 ', '96 ', '96 ', ': ', ': ', '98 ', '99 ', '98 ']),
   ('IT', ['62 ', '63 ', '69 ', '73 ', '75 ', '79 ', '81 ', '84 ', '85 ']),
   ('LT', ['60 ', '60 ', '65 ', '66 ', '68 ', '72 ', '75 ', '78 ', '82 ']),
   ('LU', ['91 ', '93 ', '94 ', '96 ', '97 ', '97 ', '97 ', '93 ', '95 ']),
   ('LV', ['64 ', '69 ', '72 ', '73 ', '76 ', '77 ', '79 ', '82 ', '85 ']),
   ('ME', [': ', '55 ', ': ', ': ', ': ', ': ', '71 ', '72 ', '74 ']),
   ('MK', [': ', '58 ', '65 ', '68 ', '69 ', '75 ', '74 ', '79 ', '82 ']),
   ('MT', ['75 ', '77 ', '78 ', '80 ', '81 ', '81 ', '85 ', '84 ', '86 ']),
   ('NL', ['94 ', '94 ', '95 ', '96 ', '96 ', '97 ', '98 ', '98 ', '98 ']),
   ('NO', ['92 ', '93 ', '94 ', '93 ', '97 ', '97 ', '97 ', '96 ', '98 ']),
   ('PL', ['67 ', '70 ', '72 ', '75 ', '76 ', '80 ', '82 ', '84 ', '87 ']),
   ('PT', ['58 ', '61 ', '62 ', '65 ', '70 ', '74 ', '77 ', '79 ', '81 ']),
   ('RO', ['47 ', '54 ', '58 ', '61 ', '68 ', '72 ', '76 ', '81 ', '84 ']),
   ('RS', [': ', ': ', ': ', ': ', '64 ', ': ', '68 ', '73 ', '80 ']),
   ('SE', ['91 ', '92 ', '93 ', '90 ', '91 ', '94 ', '95 ', '93 ', '96 ']),
   ('SI', ['73 ', '74 ', '76 ', '77 ', '78 ', '78 ', '82 ', '87 ', '89 ']),
   ('SK', ['71 ', '75 ', '78 ', '78 ', '79 ', '81 ', '81 ', '81 ', '82 ']),
   ('TR', [': ', '47 ', '49 ', '60 ', '70 ', '76 ', '81 ', '84 ', '88 ']),
   ('UK', ['83 ', '87 ', '88 ', '90 ', '91 ', '93 ', '94 ', '95 ', '96 ']),
   ('XK', [': ', ': ', ': ', ': ', ': ', ': ', '89 ', '93 ', '93 ']),
]

dictionar=dict(dataset)
df=pd.DataFrame(dictionar, index=['2011','2012','2013 ', '2014 ', '2015 ', '2016 ', '2017', '2018', '2019 '])
df.to_csv('tema5.csv')
#print(df)

#exercitiul 2

df.replace(to_replace=': ',value=0, inplace=True)
df.replace(to_replace=':',value=0,inplace=True)
#print(df)

#exercitiul 3
df=df.transpose()
df['medie']=df.astype('int').mean(axis=1)
#print(df)
df=df.transpose()
#exercitiul 4
#print(df.describe())

#exercitiul 5

#df.plot(kind='scatter', x='NL', y='NO')
#plt.show()

#exercitiul 6
#df['RO'].plot(kind='hist')
#plt.show()

#print(df.iloc[:,0])
#print(df)
#exercitiul 1 tema
#print(df.loc[['2011']])

#print(B['AL'])
#print(B[B.columns[0]])

#exercitiul unu tema
#print(df.index[8])
x=list(df.index)
print(x)
#year = input()
def get_year_data(dataset,year):
   dictionar = dict(dataset)
   df = pd.DataFrame(dictionar, index=['2011', '2012', '2013 ', '2014 ', '2015 ', '2016 ', '2017', '2018', '2019 '])
   year='2011'
   df.to_csv('tema5.csv')
   df.replace(to_replace=': ', value=0, inplace=True)
   df.replace(to_replace=':', value=0, inplace=True)
   B = df.loc[[year]]
   lista=list()
   #print(B)
   b1=list(B.columns)
   #print(B.iloc[0,0])
   dictionar_final=dict()
   #print(b1)
   q=0
   for elem in b1:
      lista1=list([elem,B.iloc[0,q]])
      lista.append([elem,B.iloc[0,q]])
      q+=1
      lista1=[]
   dictionar_final['2011'] = lista
   return dictionar_final
rezultat = get_year_data(dataset,'2018')


# exercitiul doi tema
country=input()
def get_country_data(dataset,country):
   dictionar = dict(dataset)

   df = pd.DataFrame(dictionar, index=['2011', '2012', '2013 ', '2014 ', '2015 ', '2016 ', '2017', '2018','2019'])
   df.replace(to_replace=': ', value=0, inplace=True)
   df.replace(to_replace=':', value=0, inplace=True)
   lista=list(df.index)
   lista_finala=[]
   for i in range(len(lista)):
      val=df[country].loc[lista[i]]
      lista_finala.append([lista[i],val])
   DICT=dict()
   DICT[country]=lista_finala
   return DICT
DICTIONAR=get_country_data(dataset,country)
print(DICTIONAR)

country=input()
def perform_average(*args):
   dictionar1 = dict(dataset)
   df = pd.DataFrame(dictionar, index=['2011', '2012', '2013 ', '2014 ', '2015 ', '2016 ', '2017', '2018','2019'])
   df.replace(to_replace=': ', value=0, inplace=True)
   df.replace(to_replace=':', value=0, inplace=True)
   lista=list(df.index)
   country='RO'
   lista_finala=[]
   s=0
   for i in range(len(lista)):
      val=df[country].loc[lista[i]]
      lista_finala.append([lista[i],val])
      s+=int(df[country].loc[lista[i]])
   med=s/len(lista)
   DICT=dict()
   DICT[country]=lista_finala
   return DICT,med
DICTIONAR,medie=perform_average(dataset,country)
print(DICTIONAR)
print(medie)